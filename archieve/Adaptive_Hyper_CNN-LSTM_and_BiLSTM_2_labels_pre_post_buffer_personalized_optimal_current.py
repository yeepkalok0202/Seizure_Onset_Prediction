...
BASE_SENSORS = ['ACC', 'EDA', 'HR', 'TEMP'] # Exclude BVP as requested
...
def train_one_epoch(
    ...
    if class_weights is not None:
        ...
        # Map to tensor weights ordered by class index [weight_0, weight_1]
        weight_tensor = torch.tensor(
            [class_weights.get(0, 1.0), class_weights.get(1, 1.0)], dtype=torch.float32
        ).to(device)
        # BCELoss `weight` argument should be the same shape as the target (labels)
        # which is (batch_size, 1). We need `pos_weight` for imbalanced binary cross-entropy
        # Or define a custom weighted BCELoss.
        # Let's stick to the standard BCELoss and handle weights outside or use `pos_weight`.
        # For `BCELoss`, it is more common to use `pos_weight` which is weight for the positive class.
        ...
        if 0 in class_weights and 1 in class_weights and class_weights[0] > 0:
            ...
            # Re-define criterion to use pos_weight
            criterion = nn.BCEWithLogitsLoss(
                pos_weight=pos_weight
            )  # Use BCEWithLogitsLoss for pos_weight

        else:
            # If weights are invalid or only one class, use standard BCELoss
    ...
    for inputs, labels in train_dataloader:
        ...
        # Ensure criterion matches output/label shapes (BCELoss expects probabilities, BCEWithLogitsLoss expects logits)
        # Our models use final Sigmoid, so outputs are probabilities. BCELoss is correct.
        # If we used BCEWithLogitsLoss, we would need to remove the final Sigmoid from the model.
        # Let's keep BCELoss and the final Sigmoid. If using class_weights, calculate pos_weight and use BCEWithLogitsLoss.
        # Revert criterion back to BCELoss if pos_weight was not applied
        if "pos_weight" not in locals() and not (
            isinstance(criterion, nn.BCEWithLogitsLoss)
        ):
            criterion = nn.BCELoss()  # Ensure BCELoss is used if no pos_weight
...
def evaluate_pytorch_model(model, dataloader, criterion, device):
    ...
    all_labels = []
    all_predictions = []
    all_probs = [] # Keep collecting probabilities

    # Handle empty dataloader gracefully
    if len(dataloader.dataset) == 0:
        metrics = calculate_metrics(all_labels, all_predictions, all_probs) # Returns default 0 metrics
        # Return empty lists for probs/labels in this case
    ...
    with torch.no_grad():
        for inputs, labels in dataloader_tqdm:
            ...
            outputs = model(inputs) # Models currently output probabilities (after final Sigmoid)

            # Use BCELoss for evaluation as models output probabilities
            ...
            all_probs.extend(outputs.cpu().numpy()) # Store probabilities for AUC
    ...
    # --- Modify the return statement to include all_probs and all_labels ---
...
def train_pytorch_model(
    ...
    )  # min_lr slightly lower than personalization LR
    ...
    if len(train_dataloader.dataset) == 0:
        ...
        # Evaluate on all sets with the initial model state and return metrics
    ...
    for epoch in epoch_tqdm:
        ...
        # Pass class_weights to train_one_epoch
        ...
        if len(val_dataloader.dataset) > 0:
            ...
            if val_loss < best_val_loss:
                best_val_loss = best_val_loss  # No change needed, already defined
                epochs_without_improvement = 0
                best_model_state = (
                    model.state_dict()
    ...
    # Load best weights before returning, if a best state was saved
    ...
    # Use nn.BCELoss for evaluation as models output probabilities
