...
def plot_probability_distribution(all_probs, all_labels, title, save_path):
    ...
    try:
        ...
        df_probs = pd.DataFrame({
            ...
            'True Label': ['Pre-ictal (1)' if label == 1 else 'Interictal (0)' for label in all_labels]
        })
...
if __name__ == "__main__":
    ...
    if ENABLE_TUNABLE_HYPERPARAMETERS:
        ...
        hp_param_lists = {
            ...
            "general_model_weight_decay": TUNABLE_WEIGHT_DECAY_GENERAL, # <--- ADD THIS LINE
            "personalization_weight_decay": TUNABLE_WEIGHT_DECAY_PERSONALIZATION, # <--- ADD THIS LINE
        }
        ...
        hp_combinations.append(
            {
                ...
                'dropout_rate':TUNABLE_DROPOUT_RATE[0],
                "general_model_weight_decay": TUNABLE_WEIGHT_DECAY_GENERAL[0], # <--- ADD THIS LINE
                "personalization_weight_decay": TUNABLE_WEIGHT_DECAY_PERSONALIZATION[0], # <--- ADD THIS LINE
            }
        )
    ...
    for hp_idx, current_hp_combo in enumerate(hp_combinations):
        ...
        model_hyperparameters = {
            ...
            "dropout_rate": current_dropout_rate, # <--- ADD THIS LINE
        }
        general_hyperparameters = {
                    ...
                    if (
                        ...
                        if (
                            ...
                            overall_general_model_save_path = os.path.join(
                                ...
                                f"overall_general_model_hyper_{hp_idx}.pth",
                            )
                            ...
                            (
                                ...
                                device=DEVICE,
                                weight_decay=current_general_model_weight_decay, # <--- PASS weight_decay HERE
                            )
                            if 'history' in overall_general_metrics:
                                plot_training_history(
                                    overall_general_metrics['history'],
                                    f'Overall General Model ({current_model_type}, {combination_name}, HP {hp_idx})',
                                    plot_dir_og,
                                    f'{timestamp_str}_{hp_idx}_overall_general'
                                )
                            ...
                            if 'all_probs' in overall_general_test_metrics and 'all_labels' in overall_general_test_metrics:
                                plot_auc_roc(
                                    ...
                                    f'Overall General Model AUC-ROC ({current_model_type}, {timestamp_str}, {combination_name}, HP {hp_idx})',
                                    os.path.join(plot_dir_og, f'{timestamp_str}_{hp_idx}_overall_general_auc_roc.png')
                                )
                                plot_probability_distribution(
                                    overall_general_test_metrics['all_probs'],
                                    overall_general_test_metrics['all_labels'],
                                    f'Overall General Model Probability Distribution (Test Set) ({current_model_type}, {combination_name}, HP {hp_idx})',
                                    os.path.join(plot_dir_og, f'{timestamp_str}_{hp_idx}_overall_general_prob_dist.png')
                                )
                            else:
                            ...
                            plot_confusion_matrix(
                                ...
                                ['Interictal (0)', 'Pre-ictal (1)'], # Class names
                                f'Overall General Model Confusion Matrix ({current_model_type},  {timestamp_str}, {combination_name}, HP {hp_idx})',
                                os.path.join(plot_dir_og, f'{timestamp_str}_{hp_idx}_overall_general_confusion_matrix.png')
                            )
    ...
    try:
        ...
        logging.error(f"An error occurred while writing the summary file: {e}")
