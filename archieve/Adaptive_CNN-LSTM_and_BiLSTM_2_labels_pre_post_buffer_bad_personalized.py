...
OUTPUT_DIR = 'processed_data_pytorch_adaptive_pre_post_buffer'
...
def synchronize_and_merge_data(sensor_data_dict, target_freq_hz=1):
    ...
    if merged_df.empty or merged_df.isnull().values.all(): # Check if it's empty or all NaNs after reindexing
        # print("Warning: Merged DataFrame is empty or all NaNs after reindexing. Skipping scaling.") # Suppress frequent warning
    ...
    if not valid_cols.empty:
        ...
        # print("Warning: No valid columns left to scale after reindexing and NaN check.") # Suppress frequent warning
...
def create_labeled_segments(synced_df, annotations_df, segment_duration_sec, pre_ictal_window_min, pre_ictal_exclusion_buffer_min, post_ictal_buffer_min, target_freq_hz):
    ...
    if synced_df is None or synced_df.empty or len(synced_df.columns) != len(BASE_SENSORS):
        # print("Synced data is empty, has wrong number of columns, or annotations are missing. Cannot create segments.") # Suppress frequent print
        # Return with default shape on error
        num_features = len(BASE_SENSORS) # Default to BASE_SENSORS features
        return np.array([]).reshape(0, segment_length_steps, num_features), np.array([])
    ...
    if segment_length_steps <= 0:
        ...
        num_features = synced_df.shape[1] if synced_df is not None else len(BASE_SENSORS) # Fallback
...
def prepare_patient_data(patient_folder, pre_ictal_window_min, pre_ictal_exclusion_buffer_min, post_ictal_buffer_min, sampling_freq_hz):
    """
    ...
    synced_df = synchronize_and_merge_data(sensor_data_dict, target_freq_hz=sampling_freq_hz) # Use sampling_freq_hz parameter
    ...
    segments, labels = create_labeled_segments(
        ...
        pre_ictal_window_min=pre_ictal_window_min, # Use parameter
        pre_ictal_exclusion_buffer_min=pre_ictal_exclusion_buffer_min, # Use parameter
        post_ictal_buffer_min=post_ictal_buffer_min, # Use parameter
...
def perform_personalization_pytorch(general_model_state_dict, patients_data_for_combination_sliced, model_type, sensor_combination_name):
    ...
    for patient_id, patient_segments, patient_labels, patient_found_sensors in patient_tqdm:
        ...
        # print(f"Evaluating general model ({model_type}, {sensor_combination_name}) on patient {patient_id}'s test data (Before Personalization)...") # Suppress frequent print)
...
def get_combined_data_for_combination(processed_patients_data, sensor_combination):
    ...
    try:
        # Ensure we are using the lowercase names for indexing into the BASE_SENSORS lowercase list
        base_sensors_lower = [s.lower() for s in BASE_SENSORS]
        sensor_indices = [base_sensors_lower.index(s.lower()) for s in sensor_combination]
    ...
    for patient_id, segments_all_sensors, labels, found_sensors in processed_patients_data:
        # Check if the patient has *all* sensors required for this combination (using lowercase for comparison)
        if all(s.lower() in found_sensors for s in sensor_combination):
            ...
            # else: print(f"Skipping patient {patient_id} for combination {combination_name}: No segments or single class after slicing.") # Suppress
        # else: print(f"Skipping patient {patient_id} for combination {combination_name}: Missing required sensors.") # Suppress
...
def print_personalization_summary(personalization_results, output_file=None):
    ...
    for patient_id, results in personalization_results.items():
        ...
        if 'accuracy' in results.get('before', {}) and 'accuracy' in results.get('after', {}): # Use .get for safety
            ...
            if results['after'].get('accuracy') is not None: # Assuming if accuracy is there, other metrics are too
                ...
                cm = results.get('after', {}).get('confusion_matrix', [[0,0],[0,0]])
                if isinstance(cm, list) and len(cm) == 2 and len(cm[0]) == 2 and sum(sum(row) for row in cm) > 0:
                    ...
                    count += 1
                # else: print_func(f"--- Debug: Patient {patient_id} skipped average calculation due to empty test set/CM") # Suppress
...
if __name__ == "__main__":
    ...
    if not processed_patient_data:
        ...
        with open(output_filename, 'w') as output_file:
            ...
            output_file.write(f"Data Processing Parameters:\n" # Updated section name
                            f"  SEGMENT_DURATION_SECONDS: {SEGMENT_DURATION_SECONDS}\n"
                            f"  PRE_ICTAL_WINDOW_MINUTES: {PRE_ICTAL_WINDOW_MINUTES}\n"
                            f"  PRE_ICTAL_EXCLUSION_BUFFER_MINUTES: {TUNABLE_PRE_ICTAL_EXCLUSION_BUFFER_MINUTES}\n" # Use new parameter
                            f"  POST_ICTAL_BUFFER_MINUTES: {TUNABLE_POST_ICTAL_BUFFER_MINUTES}\n" # Use new parameter
                            f"  BASE_SENSORS: {BASE_SENSORS}\n"
                            f"  SAMPLING_FREQ_HZ: {SAMPLING_FREQ_HZ}\n")
            output_file.write(f"Model Hyperparameters:\n" # Added section for model HPs
            ...
            for current_model_type in model_types_to_run:
                ...
                for current_combination in sensor_combinations:
                    combination_lower = [s.lower() for s in current_combination] # Ensure lowercase for naming consistency
                    combination_name = "_".join(combination_lower).upper() # Consistent naming
