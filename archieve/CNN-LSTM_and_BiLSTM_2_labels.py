...
from sklearn.utils import class_weight  # <--- ADD THIS IMPORT
from torch.utils.data import DataLoader, Dataset
from tqdm.auto import tqdm  # Import tqdm for progress bars

# --- Configuration ---
DATA_ROOT_DIR = 'F:\\data_9' # Replace with the actual path to your 'data_9' folder
OUTPUT_DIR = 'processed_data_pytorch' # Directory to save intermediate processed files/models
...
def load_seizure_annotations(patient_dir):
    ...
    try:
        ...
        print(f"Warning: Annotation file not found at {annotation_file}") # Suppress frequent warning
...
def synchronize_and_merge_data(sensor_data_dict, target_freq_hz=1):
    ...
    if merged_df.empty or merged_df.isnull().values.any():
        print("Warning: Merged DataFrame is empty or still contains NaNs after interpolation/filling. Skipping scaling.")
    ...
    if not merged_df.empty and len(data_cols) > 0:
        merged_df[data_cols] = scaler.fit_transform(merged_df[data_cols])
         ...
         print("Warning: Merged DataFrame is not empty but has no data columns to scale.")
...
def create_labeled_segments(synced_df, annotations_df, segment_duration_sec, pre_ictal_window_min, interictal_buffer_min, target_freq_hz):
    ...
    if synced_df is None or annotations_df is None or synced_df.empty or len(synced_df.columns) == 0:
        print("Synced data is empty, has no columns, or annotations are missing. Cannot create segments.")
    ...
    print(f"Data range: {data_start_time} to {data_end_time}")
    ...
    for i, seizure in annotations_df.iterrows():
        ...
        if seizure_start < seizure_end:
            ...
            print(f" Seizure {i+1}: Ictal window: {seizure_start} to {seizure_end}") # Optional: too verbose

    # 2. Pre-ictal Windows
    pre_ictal_windows = []
    print(f"Defining pre-ictal windows ({pre_ictal_window_min} mins before seizure onset)...")
    for i, seizure in annotations_df.iterrows():
        seizure_start = seizure['start_time']
        ...
        if pre_ictal_start < pre_ictal_end and max(pre_ictal_start, data_start_time) <= min(pre_ictal_end, data_end_time): # Use <= for inclusive overlap check
            ...
            if not check_overlap(pre_ictal_start, pre_ictal_end, seizure_windows):
                ...
                print(f" Seizure {i+1}: Pre-ictal window added: {pre_ictal_start} to {pre_ictal_end}")
            else:
                print(f" Seizure {i+1}: Calculated pre-ictal window ({pre_ictal_start} to {pre_ictal_end}) overlaps with seizure window. Skipped.")
        else:
            print(f" Seizure {i+1}: Calculated pre-ictal window ({pre_ictal_start_uncapped} to {pre_ictal_end}) or capped window ({pre_ictal_start} to {pre_ictal_end}) is invalid or outside data range. Skipped.")
    ...
    print(f"Defining interictal exclusion windows ({interictal_buffer_min} mins buffer around seizures)...")
    ...
    print(f"Creating segments (len={segment_duration_sec}s, step={segment_duration_sec}s) from {len(synced_df)} total steps...")
    ...
    print(f"Finished segmentation. Total full-length candidate segments: {segments_total_candidates}")
    print(f" Segments skipped (ictal): {segments_skipped_ictal}")
    print(f" Segments skipped (interictal buffer, not pre-ictal): {segments_skipped_interictal_buffer}")
    print(f" Total segments included for labeling (Pre-ictal + Interictal): {len(segments)}")
    print(f" Segments labeled Pre-ictal: {np.sum(labels)}, Interictal: {len(labels) - np.sum(labels)}")
    ...
    if min_count == 0:
        print("Warning: One class has zero samples after segmentation. Cannot balance.")
    ...
    if len(pre_ictal_indices) > min_count or len(interictal_indices) > min_count:
        print(f"Balancing classes: Reducing majority class to {min_count} samples.")
        ...
        print(f"After balancing: Total segments: {len(segments)}, Pre-ictal: {np.sum(labels)}, Interictal: {len(labels) - np.sum(labels)}")
...
def prepare_patient_data(patient_folder):
    ...
    if not sensor_data_dict:
        print(f"Skipping patient {patient_id}: Could not load sensor data.") # Moved to main loop desc
        return None
...
def train_pytorch_model(model, train_dataloader, val_dataloader, epochs, learning_rate, class_weights=None, save_best_model_path=None, desc="Training"):
    ...
    for epoch in epoch_tqdm:
        ...
        if val_loss < best_val_loss:
            ...
            # print(f"EarlyStopping counter: {epochs_without_improvement} out of 5") # Log inside tqdm desc instead
    ...
    if best_model_state:
        ...
        # print(f"Restored best model weights (Val Loss: {best_val_loss:.4f})") # Log inside tqdm completion or outside
...
def train_general_model_pytorch(all_patient_data, model_type='CNN-BiLSTM'):
    ...
    if not all_patient_data:
        print("No patient data available for general training.")
        return None, {}, None
    ...
    print(f"Combined data shape: {all_segments.shape}, Labels shape: {all_labels.shape}")
    print(f"Combined data: Pre-ictal: {np.sum(all_labels)}, Interictal: {len(all_labels) - np.sum(all_labels)}")


    if len(np.unique(all_labels)) < 2:
        print("Combined data has only one class. Cannot train general model.")
        return None, {}, None
...
def perform_personalization_pytorch(general_model_state_dict, all_patient_data, model_type='CNN-BiLSTM'):
    ...
    for patient_id, patient_segments, patient_labels in patient_tqdm:
        ...
        if len(patient_segments) == 0:
            print(f"No data for patient {patient_id}, skipping personalization.")
        ...
        if len(np.unique(patient_labels)) < 2:
            print(f"Skipping personalization for patient {patient_id}: Only one class present in data.")
            ...
            general_model_instance = CNN_BiLSTM(input_channels, seq_len).to(DEVICE) # Assuming CNN-BiLSTM
            try:
                ...
                print(f"Error evaluating general model for patient {patient_id} with single class: {e}")
        ...
        print(f"Patient {patient_id} - Personalization Train shape: {X_train_pat.shape}, Val shape: {X_val_pat.shape}, Test shape: {X_test_pat.shape}")
        ...
        print(f"Evaluating general model on patient {patient_id}'s test data (Before Personalization)...")
        input_channels = patient_segments.shape[2]
        seq_len = patient_segments.shape[1]
        ...
        if len(classes_pat) == 2:
            ...
            print(f"Computed patient {patient_id} class weights: {class_weights_pat_dict}")
        else:
            class_weights_pat_dict = None


        # Fine-tune the model on the patient's training data
        print(f"Fine-tuning model on patient {patient_id}'s training data...")
        ...
        print(f"Evaluating personalized model on patient {patient_id}'s test data (After Personalization)...")
    ...
    if not processed_patient_data:
        ...
        with open(output_filename, 'w') as output_file:
            ...
            for current_model_type in model_types_to_run: # <--- The loop
                print(f"\n{'='*30}")
                print(f"Running process for Model Type: {current_model_type}")
                print(f"{'='*30}\n")
                ...
                if general_model is not None:
                    ...
                    for patient_id, results in personalization_results.items():
                        ...
                        if 'accuracy' in results.get('after', {}): # Use .get to handle missing 'after' key
                            ...
                            # else: print(f"--- Debug: Patient {patient_id} metrics skipped for average due to empty test set/CM")
                        # else: print(f"--- Debug: Patient {patient_id} results['after'] is missing or invalid")
