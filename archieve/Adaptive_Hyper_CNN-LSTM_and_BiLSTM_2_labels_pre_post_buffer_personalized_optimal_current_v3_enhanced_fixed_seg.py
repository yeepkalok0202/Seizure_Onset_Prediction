...
TUNABLE_CONV_KERNEL_SIZE = [
    10,  # Original and used in previous runs
    7,   # Common odd kernel size, might capture different temporal patterns
    # 5    # Smaller common odd kernel size
...
TUNABLE_GENERAL_MODEL_LR = [
    0.001,    # Original and used in previous runs
    0.0005,   # Smaller learning rate for potentially finer convergence
    # 0.002     # Larger learning rate to try escaping local minima
]  # Used for Overall General Model and LOPO General Model
...
TUNABLE_DROPOUT_RATE = [0.5] # Recommended values to start tuning
...
MAX_PATIENTS_TO_INCLUDE = (
    None  # Example: 5 (to process only the first 5 patients found)
)
...
PROCESSED_DATA_CACHE_DIR = os.path.join(OUTPUT_DIR, "F:\\seizure_cache")
PROCESSED_DATA_CACHE_PATH = os.path.join(PROCESSED_DATA_CACHE_DIR, f"all_processed_patient_data_{data_config_id}.pkl")
...
def evaluate_pytorch_model(model, dataloader, criterion, device):
    ...
    eval_criterion = nn.BCELoss() # <-- CHANGE THIS LINE BACK
    ...
    with torch.no_grad():
        for inputs, labels in dataloader_tqdm:
            ...
            loss = eval_criterion(outputs, labels) # <-- Use the eval_criterion
            running_loss += loss.item() * inputs.size(0)

            # Predictions based on probability threshold (0.5)
            predicted = (outputs > 0.5).float() # <-- Use outputs directly for thresholding
...
if __name__ == "__main__":
    ...
    # The get_data_config_identifier function (defined earlier in the script) creates a unique string
    data_config_id = get_data_config_identifier(
        FIXED_DATA_PROCESSING_CONFIG["pre_ictal_window_min"],
        FIXED_DATA_PROCESSING_CONFIG["pre_ictal_exclusion_buffer_min"],
        FIXED_DATA_PROCESSING_CONFIG["post_ictal_buffer_min"],
    ...
    if REUSE_PROCESSED_DATA and os.path.exists(PROCESSED_DATA_CACHE_PATH):
        ...
        try:
            ...
            logging.info(f"Successfully loaded {len(all_processed_patient_data)} patients from cache.")
            processed_data_loaded_from_cache = True
        except Exception as e:
            logging.error(f"Error loading cached data: {e}. Falling back to reprocessing data.")
    ...
    if not processed_data_loaded_from_cache:
        if REUSE_PROCESSED_DATA: # If flag was true but path didn't exist or load failed
            logging.info(f"Cached processed data not found or load failed. Processing data now using config: {data_config_id}")
        else: # If flag is false
            logging.info("REUSE_PROCESSED_DATA is False. Processing data without caching.")

        # The list of patient folders is already filtered by MAX_PATIENTS_TO_INCLUDE and PATIENTS_TO_EXCLUDE
        patient_dirs_to_process = all_patient_folders
        ...
        # --- MODIFIED: Added logging inside the loop for patient processing ---
        ...
        if REUSE_PROCESSED_DATA and not processed_data_loaded_from_cache: # Only save if we just processed the data
            ...
            try:
                ...
                logging.info(f"Processed data cached to: {PROCESSED_DATA_CACHE_PATH}")
            except Exception as e:
                logging.error(f"Error saving processed data cache: {e}")

    # Check if any patients were successfully processed (either loaded or newly processed)
    if not all_processed_patient_data:
        logging.error("No patient data processed successfully (either from cache or new processing). Exiting.")
        sys.exit(1)
    ...
    for hp_idx, current_hp_combo in enumerate(tqdm(hp_combinations, desc="Overall HP Combinations")):
        ...
        for current_model_type in models_to_run:
            ...
            for current_combination in sensor_combinations_to_run:
                ...
                # This function now filters from the already processed `all_processed_patient_data` list
                (
                    patients_suitable_for_combination,
                    sensor_combination_indices,
                ) = get_patients_and_indices_for_combination(
                    all_processed_patient_data,  # Use the list populated before the HP loop
                    ...
                    if (
                        ...
                        if (
                            ...
                            if "overall_general_model" in locals() and overall_general_model is not None:
                                ...
                                logging.info(
                                    f"Overall General Model Training Metrics ({current_model_type}, {combination_name}, HP {hp_idx+1}) - Train: {format_metrics_for_summary(overall_general_metrics['train'])}"
                                )
                                logging.info(
    ...
    try:
        with open(summary_output_filename, "w") as summary_file: # Use 'w' to overwrite for the final table
            ...
            if REUSE_PROCESSED_DATA:
                ...
                summary_file.write(f"  Cached Data Path: {PROCESSED_DATA_CACHE_PATH}\n")
                summary_file.write("\n")
