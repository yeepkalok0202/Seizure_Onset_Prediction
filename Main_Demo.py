...
if __name__ == '__main__':
    # All settings are defined here and passed to the processes.
    config = {
        ...
        'EMISSION_RATE_HZ': 1,
        # This single path points to your general model bundle.
    ...
    # 1. Create the shared queue that will act as the "conveyor belt"
    # maxsize prevents the preprocessor from getting too far ahead if the predictor is slow.
    tensor_queue = Queue(maxsize=10)

    # 2. Create the process for the preprocessor
    preprocessor = Process(target=preprocessing_process, args=(tensor_queue, config))

    # 3. Create the process for the predictor and UI
    predictor_ui = Process(target=launch_predictor_ui, args=(tensor_queue, config))

    print("[MainApp] Starting all processes...")
    
    # 4. Start both processes. They will now run in parallel.
    preprocessor.start()
    predictor_ui.start()

    # 5. Wait for the processes to finish (or for user to stop with Ctrl+C)
    try:
        preprocessor.join()
        predictor_ui.join()
